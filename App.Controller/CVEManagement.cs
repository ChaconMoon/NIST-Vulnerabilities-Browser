    ...
    public class CVEManagement
    {
        ...
        public virtual async Task<JsonDocument> GetCVEInfo(String softwareName)
        {
            try
            {
                ...
                _httpClient.DefaultRequestHeaders.Add("User-Agent", "My_NUnit_Project");
        ...
        /*TESTED*/
        public static CVEs FormatJSONCVEs(JsonDocument doc_cve)
        {
            CVEs cves = new CVEs();
            foreach (JsonElement vulnerability in doc_cve.RootElement.GetProperty("vulnerabilities").EnumerateArray())
            {
                ...
                try
                {
                    impactScore = float.Parse(vulnerability.GetProperty("cve").GetProperty("metrics").GetProperty("cvssMetricV2").EnumerateArray().ElementAt(0).GetProperty("cvssData").GetProperty("baseScore").ToString(),CultureInfo.InvariantCulture);
                } 
                catch
                {
